...
minimization_routine <- function(model, erf = RT_ErrorFcn) {
  ...
  out <- list(fit = cbind(subject=j,fit),
              pvec =c(subject = j, best))
  return(out)  
}
...
testing_routine <- function(model, erf = RT_ErrorFcn) {
  err <- erf(free = model$free[[j]], fixed = model$fixed, 
             obs = model$data[model$data$subject == j,])
  fit <- data.frame(t(c(model$free[[j]], value = err)))
  out <- list(fit = cbind(subject=j,fit),
              pvec =c(subject = j, model$free[[j]]))
}
...
preds <- lapply(lapply(results, `[[`, 'pvec'),CFR_PCL,
                fixed = model$fixed, summarised=FALSE)
