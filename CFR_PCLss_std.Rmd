...
RT_ErrorFcn <- function(free, fixed, obs, fcn = CFR_PCL) {
  ...
  if (!identical(unique(dist$order),1:p['nItems'])) {
    dist <- left_join(expand.grid(class=c("np","sp","tp"),
                                  1:p['nItems'],
                                  RT=seq(.1,time,.1)),
                      dist,
                      by = c("class","order","RT"))
...
free <- list(c(ER=.62,LR=.025,Ta=150,TR=.02,Tmin=1,Tmax=45,lambda=.5), #1
             ...
             c(ER=.321,LR=0.112,Ta=132.351,TR=0.068,Tmin=1.043,Tmax=50.838,lambda=0.699), #6
