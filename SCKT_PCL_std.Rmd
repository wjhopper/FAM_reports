---
title: "SCKT PCL Meta Ss Standard Model"
author: "Will Hopper"
date: "`r Sys.Date()`"
output: 
  html_document:
...
if (!file.exists(file.path("data",paste(resultsFile,'.rds',sep='')))) {
  model <- list(free = c(ER=.52,LR=.2,TR =.03, F1=.05,space=.03),
                fixed= c(theta=.5,nFeat=100,nSim=1000,nList=48),
                fn = SCKT_PCL, 
                method="Nelder-Mead",
                ...
                name = model_prefix[2])
  results <- fitSCKT_PCL(model,inpar=params$inpar)
  saveRDS(results,
